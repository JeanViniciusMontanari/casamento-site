          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
            {gifts.map((gift) => {
              ...
              return (
                <div
                  key={gift.id}
                  className="border border-[#eadcc7] rounded-[20px] overflow-hidden shadow-md bg-white"
                >
                  <img
                    src={gift.image}
                    alt={gift.name}
                    className={
                      isPix
                        ? 'w-full h-32 md:h-40 object-contain bg-white p-4'
                        : 'w-full h-32 md:h-40 object-contain bg-white p-3'
                    }
                  />

                  <div className="p-3 md:p-4">
                    <h3 className="text-lg md:text-xl leading-6 mb-2">
                      {gift.name}
                    </h3>

                    <p className="text-base md:text-lg mb-2">{gift.value}</p>

                    {isPix && (
                      <p className="text-xs md:text-sm mb-3 leading-6 text-[#7a5b3a]">
                    ...
                    {reservedBy ? (
                      <button
                        type="button"
                        disabled
                        className="w-full bg-gray-400 text-white py-2.5 rounded-xl cursor-not-allowed text-sm"
                      >
                        Presente Reservado
                      </button>
                    ) : (
                      <>
                        <button
                          ...
                          className="w-full bg-[#8a5b2b] hover:bg-[#74491f] text-white py-2.5 rounded-xl transition-all text-sm font-semibold"
                        >
                        ...
                        {gift.link && !isPix && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mt-2 block w-full text-center bg-[#f7efe3] border border-[#caa36d] text-[#8a5b2b] py-2.5 rounded-xl transition-all text-sm font-semibold hover:bg-white"
                          >
                            Comprar na Havan
                          </a>
                        )}
      ...
      {activeSection === 'historia' && (
        <Section id="conteudo" title="Nossa História" onBack={goHome}>
          <p className="text-base md:text-lg leading-8 md:leading-9 text-center">
            ...
            repousarei. Teu Deus será o meu Deus. Teu caminho o meu será" - Rute 1:16-17
          </p>
        </Section>
      )}
    </div>
...
function Section({
  ...
  return (
    <section
      id={id}
      className={`${wide ? 'max-w-7xl' : 'max-w-4xl'} mx-auto px-4 md:px-6 py-10 md:py-20`}
    >
